-- DealPlate no-auth prototype seed for Supabase/PostgreSQL.
-- Run after schema.sql when you want open prototype access without Supabase Auth users.
...
DROP POLICY IF EXISTS "Prototype allow all on vendors" ON public.vendors;
DROP POLICY IF EXISTS "Prototype allow all on student_profiles" ON public.student_profiles;
DROP POLICY IF EXISTS "Prototype allow all on deals" ON public.deals;
DROP POLICY IF EXISTS "Prototype allow all on orders" ON public.orders;
...
CREATE POLICY "Prototype allow all on vendors"
ON public.vendors
FOR ALL
USING (true)
WITH CHECK (true);

CREATE POLICY "Prototype allow all on student_profiles"
ON public.student_profiles
FOR ALL
USING (true)
WITH CHECK (true);

CREATE POLICY "Prototype allow all on deals"
ON public.deals
FOR ALL
USING (true)
WITH CHECK (true);

CREATE POLICY "Prototype allow all on orders"
ON public.orders
FOR ALL
USING (true)
WITH CHECK (true);
...
GRANT EXECUTE ON FUNCTION public.decrement_stock(uuid) TO anon, authenticated;
