...
export default function InventoryClient({ 
    ...
    const buildPayload = () => {
        ...
        if (!editingId) {
            ...
            if (originalItem && form.isPublished) {
                const wasExpired = new Date(originalItem.expiresAt).getTime() <= Date.now();
                if (wasExpired) {
                    // They checked "Publish to Explore" on an expired item in the edit form.
                    payload.expires_at = new Date(
                        Date.now() + form.activeHours * 60 * 60 * 1000,
                    ).toISOString();
                }
    ...
    const handleDeleteProduct = async (item: InventoryItem) => {
        ...
        if (error) {
            if (error.code === "23503") {
                setMessage(
                    `Cannot delete "${item.title}" because it already has associated orders or data. Unpublish it instead.`,
                );
            } else {
                ...
                <div className="w-full">
                    {isFormOpen && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 sm:p-6 overflow-y-auto">
                            <section className="bg-white rounded-xl max-w-2xl w-full p-6 shadow-xl relative my-auto">
                        <div className="flex items-center justify-between gap-3 border-b border-[#E2E8F0] pb-4 mb-5">
                            <div>
                                ...
                                <p className="text-xs text-gray-500">
                                    Published items with stock appear in Explore.
                                </p>
                            </div>
                            <div className="flex items-center gap-2">
                                ...
                                <button
                                    type="button"
                                    onClick={closeForm}
                                    className="h-9 w-9 rounded-lg border border-[#E2E8F0] text-gray-500 flex items-center justify-center hover:bg-gray-50"
                                >
                    ...
                    <section className="bg-white rounded-xl border border-[#E2E8F0] shadow-sm overflow-hidden mt-6">
