...
export default function DealDetailsClient({
  ...
  deal: Deal | null;
  initialSavedDealIds: Set<string>;
}) {
  ...
  const handleToggleSaved = () => {
    if (!deal) return;
    if (!user) {
      router.push(`/student/sign-in?next=${encodeURIComponent(`/student/deals/${deal.id}`)}`);
      return;
    }
    toggleSaved(deal.id);
  };
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            aria-label={isSaved(deal.id) ? 'Remove from saved deals' : 'Save this deal'}
            className={cn(
              'w-10 h-10 bg-white border border-[#F3F4F6] rounded-full flex items-center justify-center transition-colors shadow-sm',
              isSaved(deal.id) ? 'text-[#E11D48]' : 'text-gray-400 hover:text-[#E11D48]'
            )}
          >
            <Heart className="w-5 h-5" fill={isSaved(deal.id) ? 'currentColor' : 'none'} />
          </button>
          <button
            onClick={handleShare}
