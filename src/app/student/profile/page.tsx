    ...
    >
      ...
      <section className="bg-white rounded-2xl border border-[#F3F4F6] p-6 shadow-sm flex flex-col items-center text-center relative">
        <div className="relative mb-4 group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
          ...
          <button className="absolute bottom-0 right-0 bg-white border border-[#F3F4F6] rounded-full p-2 text-[#FF6B00] hover:bg-[#FFF8F6] transition-colors shadow-sm active:scale-90 z-10">
        ...
        <div className="flex flex-col gap-3">
          {pastOrders.map((order) => (
            <div
              key={order.id}
              onClick={() => {
                if (order.status === 'Active') router.push('/student/order-confirmed');
              }}
