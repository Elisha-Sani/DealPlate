...
import { Camera, FileText, Loader2, ShieldAlert, UserCheck } from 'lucide-react';
...
export default function StudentUploadId() {
  ...
  const [universityDocFile, setUniversityDocFile] = useState<File | null>(null);
  ...
  useEffect(() => {
    const rawDetails = sessionStorage.getItem('dealplate_student_kyc_details');
  ...
  const handleSubmit = async () => {
    ...
    if (!studentIdFile || !universityDocFile || !documentDate) {
      setError('Upload your student ID, a recent university document, and the document date.');
      return;
    }
    if (studentIdFile.size > 2_000_000 || universityDocFile.size > 2_000_000) {
    ...
    try {
      ...
      const universityDocDataUrl = await readFileAsDataUrl(universityDocFile);
      ...
      const { error: insertError } = await supabase.from('student_kyc_applications').insert({
        student_id: user.id,
  ...
  return (
    <motion.div
      ...
      className="max-w-[520px] w-full mx-auto py-4"
    >
      <StepIndicator currentStep={2} totalSteps={2} label="KYC Documents" />
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-[#5a4136] text-sm max-w-md mx-auto">
          Upload your school ID and one university document dated within the last six months, such as a fee statement.
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-[#F3F4F6] p-6 shadow-md flex flex-col">
        ...
        <div className="space-y-4">
          <label className="block">
            <span className="block text-xs font-bold text-[#5a4136] uppercase tracking-wider mb-2">Student ID card</span>
            <input type="file" accept="image/*,.pdf" onChange={(e) => setStudentIdFile(e.target.files?.[0] || null)} className="w-full text-sm" />
          </label>
          <label className="block">
            <span className="block text-xs font-bold text-[#5a4136] uppercase tracking-wider mb-2">Recent university document</span>
            <input type="file" accept="image/*,.pdf" onChange={(e) => setUniversityDocFile(e.target.files?.[0] || null)} className="w-full text-sm" />
          </label>
          <label className="block">
            <span className="block text-xs font-bold text-[#5a4136] uppercase tracking-wider mb-2">Document issue date</span>
            <input type="date" value={documentDate} onChange={(e) => setDocumentDate(e.target.value)} className="w-full h-12 rounded-lg border border-gray-200 bg-white px-4 text-sm focus:ring-2 focus:ring-[#FF6B00] outline-none" />
          </label>
        </div>
        ...
        <div className="bg-[#F3F4F6] rounded-xl p-4 flex gap-3 border border-gray-100 mt-5">
          ...
          <p className="text-xs text-[#5a4136] leading-relaxed">
            AI checks document clarity, institution, registration number, name match, and date recency. A superadmin still makes the final decision.
          </p>
        </div>
