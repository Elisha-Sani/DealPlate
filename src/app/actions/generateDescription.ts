...
export async function generateDealDescription(params: GenerateDescriptionParams) {
  try {
    ...
    const prompt = `
      ...
      Output ONLY a raw JSON object (do not wrap in markdown \`\`\`json block) with the following format:
      {
        ...
        "detailedDescription": "A detailed 2-3 sentence paragraph ${params.isMysteryBag
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      ...
      console.error(`Gemini API Error (Attempt ${attempt + 1}/${maxRetries}):`, errText);
