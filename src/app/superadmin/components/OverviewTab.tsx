...
export function OverviewTab({ overviewStats }: { overviewStats: AdminOverviewStats | null }) {
  return (
    <section className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
      <StatCard label="Total Students" value={overviewStats?.totalStudents ?? 0} />
      ...
      <StatCard label="Total Vendors" value={overviewStats?.totalVendors ?? 0} />
      ...
      <StatCard label="Open Support Queries" value={overviewStats?.openSupportTickets ?? 0} accent="text-[#FF6B00]" />
    </section>
  );
}
