...
export default function SuperadminClient({
  ...
  const [studentApps, setStudentApps] = useState<StudentKycApplication[]>(initialStudentApps);
  ...
  useEffect(() => {
    setStudentApps(initialStudentApps);
  ...
  return (
    <main className="min-h-screen bg-[#F9FAFB] text-[#1E293B] p-6">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="max-w-7xl mx-auto space-y-6">
        ...
        {/* Layout Container */}
        <div className="flex flex-col-reverse md:flex-row-reverse gap-6 items-start relative">
          {/* Tab bar / Sidebar */}
          <div className="flex md:flex-col gap-1 bg-white border border-[#E2E8F0] rounded-xl p-1.5 w-full md:w-64 overflow-x-auto shrink-0 md:sticky md:top-6 md:max-h-[calc(100vh-3rem)]">
            {TABS.map((tab) => (
              <button
                key={tab.id}
