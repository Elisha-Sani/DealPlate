...
export default function OrderQueueTable({ orders }: OrderQueueTableProps) {
  ...
  const awaitingOrders = orders.filter(o => o.status === 'Active');
  const collectedOrders = orders.filter(o => o.status === 'Completed');
        ...
        <table className="w-full text-left">
          <thead className="bg-white text-xs font-bold text-gray-500 uppercase tracking-wider border-b border-[#E2E8F0]">
            <tr>
              ...
              <th className="px-6 py-4 font-semibold">Deal Title</th>
          ...
          <tbody className="divide-y divide-[#E2E8F0]">
            {orders.map((order) => (
              <tr key={order.id} className="hover:bg-gray-50/50">
                <td className="px-6 py-4 text-sm text-gray-500 font-mono" title={order.id}>
                  #{order.id.slice(0, 8).toUpperCase()}
                </td>
                <td className="px-6 py-4">
                  <div className="font-bold text-[#1E293B]">{order.student?.full_name || 'Unknown Student'}</div>
                  <div className="text-xs text-gray-500">{order.student?.phone || 'No phone'}</div>
                </td>
                <td className="px-6 py-4">
